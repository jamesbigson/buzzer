      ...
      try {
        ...
        switch (data.type) {
          ...
          case 'release_buzzers': {
            ...
            // Store the server timestamp for accurate timing
            const serverTimestamp = Date.now();
            await storage.setRoomTimerStart(roomCode, serverTimestamp);
          ...
          case 'reset_buzzers': {
            ...
            // Clear buzz results and timer start
            await storage.clearBuzzResultsByRoomCode(roomCode);
            await storage.clearRoomTimerStart(roomCode);
          ...
          case 'buzz_in': {
            ...
            // Get the timer start timestamp for this room
            const timerStart = await storage.getRoomTimerStart(roomCode);
            if (!timerStart) {
              ws.send(JSON.stringify({
                type: 'error',
                message: 'Buzzer not released yet'
              }));
              break;
            }
            
            // Calculate server-side timing for accuracy
            const buzzTimestamp = Date.now();
            const buzzTime = Math.max(0, (buzzTimestamp - timerStart) / 1000);
            
            // Record buzz result with server timing
            ...
            // Acknowledge to the player with server-calculated time
            ws.send(JSON.stringify({
              type: 'buzz_acknowledged',
              time: result.time,
              serverTimestamp: buzzTimestamp
            }));
          ...
          case 'kick_player': {
            ...
            const playerToKick = await storage.getPlayerBySocketId(playerId);
            if (!playerToKick) {
              ws.send(JSON.stringify({
                type: 'error',
                message: 'Player not found'
            ...
            wss.clients.forEach((client) => {
              if (client.readyState === WebSocket.OPEN && 
                  (client as any).socketId === playerId) {
                client.send(JSON.stringify({
            ...
            // Get remaining players and notify everyone
