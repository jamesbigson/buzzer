...
export interface IStorage {
  ...
  setRoomTimerStart(roomCode: string, timestamp: number): Promise<void>;
  getRoomTimerStart(roomCode: string): Promise<number | undefined>;
  clearRoomTimerStart(roomCode: string): Promise<void>;
}
...
export class MemStorage implements IStorage {
  ...
  private roomTimerStarts: Map<string, number>; // Track timer start timestamps per room
  ...
  async setRoomTimerStart(roomCode: string, timestamp: number): Promise<void> {
    this.roomTimerStarts.set(roomCode, timestamp);
  }

  async getRoomTimerStart(roomCode: string): Promise<number | undefined> {
    return this.roomTimerStarts.get(roomCode);
  }

  async clearRoomTimerStart(roomCode: string): Promise<void> {
    this.roomTimerStarts.delete(roomCode);
  }
}
